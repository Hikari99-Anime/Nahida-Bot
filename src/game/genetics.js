...
// Gene cây gốc trong database: 10 -> 100
const GENE_MIN = 10;
const GENE_MAX = 100;
...
/**
 * Kiểm tra một plant có phải cây lai hay không.
 *
 * Dùng nhiều lớp:
 *
 * 1. isHybridPlant() từ ./plants
 * 2. ID bắt đầu bằng hybrid_
 * 3. parent_a / parent_b tồn tại
 */
function isHybrid(parent) {
    if (!parent) {
        return false;
    }
    ...
    try {
        if (
            typeof isHybridPlant === "function" &&
            isHybridPlant(parent.id)
        ) {
            return true;
        }
    } catch (error) {
        // Bỏ qua, tiếp tục check bằng ID/database
    }
...
function geneticsEmbed(user) {
    ...
    const lines = [
        ...
        "> Cây lai không thể tiếp tục dùng để lai.",
        ""
    ];
...
// ============================================================
// GET BREEDABLE PLANTS
// ============================================================

function getBreedablePlants(userId) {
    const items =
        getInventory(userId);
    ...
    return items
        ...
        // ====================================================
        // QUAN TRỌNG:
        // KHÔNG cho cây lai xuất hiện trong menu breeding.
        // ====================================================
...
function inheritGene(
    ...
    // Regression về 50 để chống snowball
    result =
        (
            result *
            0.85
...
function createHybridPlant(
    ...
    // Không cho hybrid làm parent
    if (
        isHybrid(parentA) ||
        isHybrid(parentB)
    ) {
        throw new Error(
            "HYBRID_PARENT_NOT_ALLOWED"
        );
    }

    // Không cho cùng parent
    if (
        String(parentA.id) ===
        String(parentB.id)
    ) {
    ...
    const id =
        `hybrid_${Date.now()}_${Math.random()
            .toString(36)
            .slice(2, 9)}`;
...
async function breedPlants(
    ...
    if (
        ...
        return interaction.reply({
            content:
                "❌ **Cây lai không thể dùng làm cây bố.** Hãy chọn cây gốc.",
            ephemeral: true
        });
    }
    ...
    if (
        ...
        return interaction.reply({
            content:
                "❌ **Cây lai không thể dùng làm cây mẹ.** Hãy chọn cây gốc.",
            ephemeral: true
        });
    }
    ...
    try {
        ...
        if (
            ...
            return interaction.reply({
                content:
                    "❌ **Cây lai không thể lai tiếp.** Chỉ cây gốc mới có thể dùng để lai.",
                ephemeral: true
            });
        }
    ...
    const result = [
        ...
        "",
        `> 🚫 Cây con này **không thể dùng để lai tiếp**.`
    ];
